...
"build": "tsc",                                                         //打包命令
"push": "git push Gitee master && git push origin master"               //分别推送到Gitee和Github远程仓库master分支
```
...
``` bash
git remote add origin https://github.com/ADarkDream/Download-Reverse1999.git
# and
...
或`git push Gitee master` or `git push origin master`可分别推送到两个仓库的 master 分支
